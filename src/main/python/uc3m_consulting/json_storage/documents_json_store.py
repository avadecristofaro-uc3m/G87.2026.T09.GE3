...
class DocumentsJsonStore:
    """Singleton Documents JSON store"""
    class _DocumentsJsonStore(JsonStore):
    ...
    def __new__(cls):
        if not DocumentsJsonStore.__instance:
            DocumentsJsonStore.__instance = DocumentsJsonStore._DocumentsJsonStore()
        return DocumentsJsonStore.__instance

    def load(self):
        """Load documents"""
        return self.__instance.load()